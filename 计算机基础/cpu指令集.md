> 参考：https://juejin.cn/post/6945033887462457351#heading-2

## 概念

- 首先我们知道代码编译的流程是`代码 -> 词法语法分析 -> 语义分析 -> 中间代码生成 -> 目标代码生成` , 生成的目标代码想要被 CPU 执行，就需要使用 CPU 提供的 API，这个 API 就是 **CPU 指令集**

## 常见分类

1. **复杂指令集（CISC）**:指令数量多且复杂，指令长度不相同，硬件实现复杂指令，属于复杂指令集的处理器包括x86、x86_64等
2. **精简指令集（RISC）**:指令少且简单，指令等长，编译器进行指令组合完成复杂操作，属于RISC的处理器包括**arm**等
...
> 参考：https://blog.csdn.net/bjbz_cxy/article/details/79467688

- MOV - 传送指令,在两个寄存器或寄存器和内存之间传送数据
- PUSH/POP - 将数据压入/弹出栈
- XCHG - 交换两个寄存器中的内容
- ADD/SUB - 整数加法/减法运算

- AND/OR/XOR/NOT - 逻辑与/或/异或/非运算

- CALL/RET - 调用/返回子程序

- CALL/RET - 调用/返回子程序

- MOVS/SCAS - 串操作,移动/扫描字符串
